...
cluster_vars <- c("RXLEV_dBm", "SNR_dB", "DL_Speed_kbps", "RXQUAL", "Physical_Speed", "Height_m", "Population_Density")
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (!"All" %in% input$carrier_filter) {
      data <- data %>% filter(Carrier %in% input$carrier_filter)
    }
    if (!"All" %in% input$band_filter) {
